...
class Chronos(object):
    ...
    def do_real_arbitrage(self, lower_exchange, higher_exchange, excel, spread_tracker, coin, refund):
        """
    ...
    def arbitrage_buy(self, lower_exchange,lower_exchange_transactions, higher_exchange, higher_exchange_transactions, coin, coin_id, spread_tracker, refund):
        ask = getattr(lower_exchange, "{0}_USD_ask".format(coin))
        log.debug("{0} price is {1}".format(lower_exchange.name, ask))
        ...
        spread_tracker.get_spread(bid, current_price,
                                  ...
                                  float(self.config[higher_exchange.name]['{0}_withdraw_fees'.format(coin)]),
                                  float(self.config['chronos']['wallet_exposure']))
    ...
    def arbitrage_sell(self, lower_exchange, higher_exchange, higher_exchange_transactions, coin, coin_id, higher_shares, refund):
        ...
        current_price = higher_exchange.get_limit_price('bids', higher_shares, self.config[higher_exchange.name][coin])
        ...
        while available == 0:
            ...
            higher_exchange_transactions.cancel_order(response)
            current_price = higher_exchange.get_limit_price('asks', higher_shares, self.config[higher_exchange.name][coin])
            if not refund:
                current_price = current_price + .02
            else:
                current_price = current_price - .01
